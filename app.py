...
def predict():
    ...
    if len(kp_sequence) == MAX_LENGTH_FRAMES:
        ...
        # Check for all-zero sequences and analyze data quality
        kp_zeros = np.count_nonzero(kp_sequence) == 0
        metrics_zeros = np.count_nonzero(metrics_seq) == 0
        kp_variance = np.var(kp_sequence)
        metrics_variance = np.var(metrics_seq)
        print(f"kp_sequence all zeros: {kp_zeros}, metrics_seq all zeros: {metrics_zeros}")
        print(f"kp_sequence variance: {kp_variance:.6f}, metrics_seq variance: {metrics_variance:.6f}")
        print(f"kp_sequence range: [{np.min(kp_sequence):.3f}, {np.max(kp_sequence):.3f}]")
        print(f"metrics_seq range: [{np.min(metrics_seq):.3f}, {np.max(metrics_seq):.3f}]")
        
        res = lstm_model.predict([kp_sequence, metrics_seq])[0]
        actions = get_actions(DATA_PATH)
        
        print(f"Actions available: {actions} (count: {len(actions)})")
        print(f"Predicted index: {np.argmax(res)}, Confidence: {res[np.argmax(res)]}")
        print(f"Full prediction array: {res}")

        predicted_idx = np.argmax(res)
        if res[predicted_idx] > 0.3 and predicted_idx < len(actions):
            sent = actions[predicted_idx]
            text_to_speech(sent)
            return jsonify({'prediction': sent})
        elif predicted_idx >= len(actions):
            return jsonify({'error': f'Model predicted unknown class {predicted_idx}'}), 400
